...
DEFAULT_JSON_FILE = 'dataset_ig-reels-scraper_2026-08-08_13-12-42-478.json'
DEFAULT_OUTPUT_DIR = 'random_reels_hd'
MAX_WORKERS = 10
MAX_RETRIES = 3
...
def extract_video_items(raw_json_data):
    """
    Extracts all video/reel item dicts from dataset, 
    whether root posts or profile timeline edges.
    """
    items = []
    seen_codes = set()

    def add_node(node):
        code = node.get('code') or node.get('shortcode') or node.get('id') or extract_code_from_url(node.get('video_url')) or f"vid_{len(items)+1}"
    ...
    if isinstance(raw_json_data, list):
        for item in raw_json_data:
            ...
            if kind == 'profile':
                ...
                edges = raw.get('edge_owner_to_timeline_media', {}).get('edges', []) if isinstance(raw, dict) else []
                for edge in edges:
                    n = edge.get('node', {})
                    if n.get('is_video') or n.get('video_url') or n.get('dash_info') or n.get('video_versions'):
                        add_node(n)
            else:
                if item.get('is_video') or item.get('video_url') or item.get('video_versions') or item.get('dash_info') or item.get('post_type') in ('reel', 'video', 'clips', 'clip'):
                    add_node(item)
    elif isinstance(raw_json_data, dict):
        add_node(raw_json_data)
...
def is_suitable_for_meta(item):
    ...
    1. Must contain a valid video URL, video versions, or DASH manifest.
    ...
    """
    has_video = bool(item.get('video_url') or item.get('video_versions') or item.get('dash_info') or (isinstance(item.get('raw_data'), dict) and item['raw_data'].get('video_dash_manifest')))
    if not has_video:
        return False, "No video URL, versions, or DASH manifest found"
...
def extract_dash_candidates(dash_manifest):
    candidates = []
    ...
    try:
        ...
        for rep in root.iter('{urn:mpeg:dash:schema:mpd:2011}Representation'):
            ...
            if 'video' in mime or rep.attrib.get('width'):
...
def get_hd_video_candidates(item):
    ...
    # 2. From dash_info or raw_data -> video_dash_manifest
    dash_manifest = None
    if item.get('dash_info'):
        dash_manifest = item.get('dash_info', {}).get('video_dash_manifest')
    if not dash_manifest and isinstance(item.get('raw_data'), dict):
        dash_manifest = item.get('raw_data', {}).get('video_dash_manifest')
    
    dash_cands = extract_dash_candidates(dash_manifest)
...
def download_video(item_info, output_dir):
    idx, item = item_info
...
def main():
    parser = argparse.ArgumentParser(description="HD Instagram Reels Video Downloader following download_kucing_hd.py standards.")
    parser.add_argument("-i", "--input", default=DEFAULT_JSON_FILE, help="Path to JSON dataset file")
    parser.add_argument("-o", "--output", default=DEFAULT_OUTPUT_DIR, help="Output directory to save HD videos")
    parser.add_argument("-w", "--workers", type=int, default=MAX_WORKERS, help="Number of concurrent worker threads")
    ...
    data = extract_video_items(raw_data)
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        futures = {executor.submit(download_video, item, output_dir): item[0] for item in items}
